<x-admin-layout>
    ...
    <div class="container mx-auto mt-5 p-5">
        ...
        <table class="cell-border w-full datatable-list" id="categories">
    ...
    @section('scripts')
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                let route = '{{route('api.categories',false)}}';

                window.drawDataTable($datatable, route, columns);

                window.registerEditButton($datatable, '/admin/edit-category');

                window.registerDisableButton($datatable, '/admin/disable-category');

                window.registerEnableButton($datatable, '/admin/enable-category');

                window.registerShowDisabledChanged($datatable, '{{route('api.categories')}}', '{{route('api.categories.show-disabled')}}');
            });
        </script>
    @endsection
</x-admin-layout>
